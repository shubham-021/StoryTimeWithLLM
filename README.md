...
- then navigate to core , run pnpm install here again , just to be sure
- in there run docker-compose up -d
- then run pnpm prisma migrate and pnpm prisma generate
- then update .env file with your OPENAI_API_KEY and run pnpm start (this should start the server , and should log `Server running on port 8080`)
